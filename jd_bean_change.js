...
async function showMsg() {
	...
	if ($.joylevel || $.jdCash || $.JoyRunningAmount) {
		...
		if ($.JoyRunningAmount) {
			...
			ReturnMessage += `汪赛跑:${$.JoyRunningAmount}元`;
		}
			...
			try {
				if (err) {
					...
					if (data) {
						...
						if ($.jdwxRed > 0)
							$.message += `【微信红包】${$.jdwxRed}(将过期${$.jdwxRedExpire.toFixed(2)})元 \n`;
						if ($.jdGeneralRed > 0)
							$.message += `【平台通用】${$.jdGeneralRed}(将过期${$.jdGeneralRedExpire.toFixed(2)})元 \n`;
					} else {
						console.log(`京东服务器返回空数据`)
					}
				}
